 ...
 * { ..., date: new Date(), ... }
 */
function parseArrayOfObjects(data, dateFormat, localization) {
    if (isArray_1.default(data)) {
        var parsed = data.slice();
        parsed.forEach(function (item) {
            item.dates = item.dates.map(function (date) { return parseValue(date, dateFormat, localization); });
        });
        return parsed;
    }
    return [];
}
exports.parseArrayOfObjects = parseArrayOfObjects;
/** Create moment.
 *
...
function parseDatesRange(inputString, dateFormat, inputSeparator) {
    ...
    var dates = inputString.split(inputSeparator).map(function (date) { return cleanDate(date, dateFormat); });
    var result = {};
    var start;
    var end;
